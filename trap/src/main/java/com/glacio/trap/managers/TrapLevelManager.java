...
public class TrapLevelManager {
    ...
    private void loadLevelsConfig() {
        ...
        ConfigurationSection featuresSection = config.getConfigurationSection("feature-definitions");
        if (featuresSection != null) {
            for (String feature : featuresSection.getKeys(false)) {
                // Feature definitions loaded for future use
                plugin.getLogger().info("Loaded feature: " + feature);
            }
        }
    }
    ...
    public void updateTrapPrice(int trapId, double newPrice) {
        getTrapStats(trapId).setSalePrice(newPrice);
    }
    ...
    public Map<Integer, TrapStats> getTrapStatsMap() {
        return new HashMap<>(trapStatsMap);
    }
    
    public void reloadLevelsConfig() {
        levelConfig.clear();
        loadLevelsConfig();
        plugin.getLogger().info("Trap level configuration reloaded!");
    }
    
    public boolean hasFeature(int trapId, String feature) {
        TrapStats stats = getTrapStats(trapId);
        int level = stats.getLevel();
        
        Map<String, Object> levelData = levelConfig.get(level);
        if (levelData == null) return false;
        
        @SuppressWarnings("unchecked")
        java.util.List<String> features = (java.util.List<String>) levelData.getOrDefault("features", new java.util.ArrayList<>());
        
        // Check if feature is directly available
        if (features.contains(feature)) return true;
        
        // Check if "all_previous_features" is available and feature was in previous levels
        if (features.contains("all_previous_features")) {
            for (int i = 1; i < level; i++) {
                Map<String, Object> prevLevelData = levelConfig.get(i);
                if (prevLevelData != null) {
                    @SuppressWarnings("unchecked")
                    java.util.List<String> prevFeatures = (java.util.List<String>) prevLevelData.getOrDefault("features", new java.util.ArrayList<>());
                    if (prevFeatures.contains(feature)) return true;
                }
            }
        }
        return false;
    ...
    public java.util.List<String> getAvailableFeatures(int trapId) {
        TrapStats stats = getTrapStats(trapId);
        int level = stats.getLevel();
        java.util.List<String> allFeatures = new java.util.ArrayList<>();
        
        for (int i = 1; i <= level; i++) {
            Map<String, Object> levelData = levelConfig.get(i);
            if (levelData != null) {
                @SuppressWarnings("unchecked")
                java.util.List<String> features = (java.util.List<String>) levelData.getOrDefault("features", new java.util.ArrayList<>());
                
                if (features.contains("all_previous_features") && i == level) {
                    // Skip adding "all_previous_features" itself
                    continue;
                }
    ...
    public String getLevelDisplayName(int level) {
        Map<String, Object> levelData = levelConfig.get(level);
        if (levelData == null) return "&7Seviye " + level;
        
        return (String) levelData.getOrDefault("display-name", "&7Seviye " + level);
    }
}
